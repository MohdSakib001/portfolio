...
export default async function ProjectPage({ params }: Props) {
    ...
    const breadcrumbSchema = JSON.stringify({
        ...
        "@type": "BreadcrumbList",
        itemListElement: [
            { "@type": "ListItem", position: 1, name: "Home", item: "https://mohdsakib.vercel.app" },
            { "@type": "ListItem", position: 2, name: "Projects", item: "https://mohdsakib.vercel.app/projects" },
            { "@type": "ListItem", position: 3, name: project.name, item: `https://mohdsakib.vercel.app/projects/${project.id}` },
        ],
    });

    return (
        <main className="bg-white text-black">
            <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: softwareApplicationSchema }} />
            ...
            <section className="relative h-[80vh] w-full mt-4">
                ...
                <div className="absolute bottom-10 left-10 text-white">
                    <p className="text-xs uppercase tracking-[0.2em] opacity-60 mb-2">{project.category}</p>
            ...
            <section className="px-6 md:px-16 py-24 grid md:grid-cols-2 gap-16">
                <div>
                    <h2 className="text-xs uppercase tracking-[0.2em] opacity-40 mb-4">The Problem</h2>
                    <p className="opacity-70 leading-relaxed">{project.overview.problem}</p>
                </div>

                <div>
                    <h2 className="text-xs uppercase tracking-[0.2em] opacity-40 mb-4">The Solution</h2>
                    <p className="opacity-70 leading-relaxed">{project.overview.solution}</p>
                </div>

                <div>
                    <h2 className="text-xs uppercase tracking-[0.2em] opacity-40 mb-4">My Role</h2>
                    <p className="opacity-70">{project.overview.myRole}</p>
                </div>

                <div>
                    <h2 className="text-xs uppercase tracking-[0.2em] opacity-40 mb-4">Timeline</h2>
            ...
            <section className="px-6 md:px-16 py-24 border-t">
                ...
                <div className="mt-12">
                    <p className="text-xs uppercase tracking-[0.2em] opacity-40 mb-4">Tech Stack</p>
                ...
                <div className="flex flex-col gap-6">
                    {project.achievements.map((a, i) => (
                        <div key={i} className="flex gap-4">
                            <span className="text-xs opacity-30 mt-1 font-mono">{String(i + 1).padStart(2, "0")}</span>
                ...
                <div className="flex flex-wrap gap-4">
                    ...
                    {project.links.playstore && project.links.playstore !== "#" && (
                        <a href={project.links.playstore} target="_blank" rel="noreferrer">
                            <Image width={140} height={44} src="/assets/playstore.webp" alt="Get it on Google Play" className="h-12 w-auto" />
                        </a>
                    )}
                    {project.links.appstore && project.links.appstore !== "#" && (
                        <a href={project.links.appstore} target="_blank" rel="noreferrer">
                            <Image width={140} height={44} src="/assets/appstore.webp" alt="Download on the App Store" className="h-12 w-auto" />
                        </a>
                    )}
                </div>
            </section>
